...
Two set of variables has been selected to explain the diary incidence
...
constant along the temporal period analized in this research. In this
subsetion we present some evidences of the expected relationship between
the four control variables considered an the incidence ratio.
...
concentrate (with som exception) in the mediterranean coast (Goerlich
...
of the Iberian Peninsula (see figure \ref{plotfer1}).

The per capita GDP also can generate differences between provinces. No
direct relationship can be observed between the GDP and the ratio of
incidence but the GDP is a proxy of inobservable information. This
varaible has been included in similar papers (Wang et al. (2020)). For
low DGPpc is more difficult maintain social distance (sing +); High
DGPpc -\textgreater{} more economic activity -\textgreater{} es mas
difícl mantener el confinamiento.
